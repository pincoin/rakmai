        ...
        if serializer.is_valid():
            ...
            if response and response['certified']:
                ...
                try:
                    ...
                    if not logs:
                        # > 50 years old + women + joined within 60 days
                        if now().date() - datetime.strptime(log.date_of_birth, '%Y%m%d').date() \
                                > timedelta(days=365 * 50) \
                                and log.gender == 0 \
                                and now() - profile.user.date_joined < timedelta(days=60):
                            return Response(data=json.dumps({
                                'code': 400,
                                'message': str(
                                    _(
                                        'Person aged over 50 can verify your account during 60 days after joined.'))
                            }),
